...
These are exactly the hypotheses we started **Soul Spec** and **Soul Memory** with six months ago — and they are two distinct layers. **Soul Spec** is the open standard that *defines a persona*: five canonical markdown files (`SOUL`, `IDENTITY`, `AGENTS`, `TOOLS`, `USER`) plus a versioned `soul.json` manifest. **Soul Memory** is the 4-tier adaptive memory architecture that *preserves experience* on top of it — T0 SOUL (identity, immutable) / T1 Core (evergreen, no decay) / T2 Working (dated logs, 23-day half-life decay) / T3 Session (ephemeral). OpenAI's three pillars — persistent context, preference compliance, temporal understanding — map exactly onto this combination of Soul Spec (who you are) and Soul Memory (what you remember).
